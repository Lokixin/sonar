...
class AiDj:
    ...
    def select_next_track(self) -> Tuple[int, str, float, float, int]:
        ...
        transition_scores = self.similarity_matrix.loc[
            self.played_track_ids, :
        ] * np.expand_dims(weights, axis=1)

        # get probability distribution over the candidates
        transition_probabilities = pd.Series(
            data=transition_scores.values.sum(axis=0) / size,
            index=self.track_ids,
        )

        # zero probability for tracks with previously played artists
        transition_probabilities = self._apply_no_artist_repeat(
            transition_probabilities
        )

        # zero probability for tracks with disimilar bpms
        transition_probabilities = self._apply_similar_bpm(transition_probabilities)
        ...
        next_track_sim = self.similarity_matrix.loc[self.played_track_ids[-1], next_track_id]

        return next_track_id, next_track_name, next_track_sim, next_track_prob, num_possible_transitions

    def select_and_add_next_track(self) -> str:
        ...
        (
            ...
            next_track_sim,
            next_track_prob,
            num_possible_transitions,
        ) = self.select_next_track()
        self.add_track(
            track_id=next_track_id,
            dj="AI",
            similarity=next_track_sim,
            transition_prob=next_track_prob,
            num_possible_transitions=num_possible_transitions,
        )

        return next_track_name
